...
public class test1 extends Applet{
	...
	//La taille de cube, quand on cr�e un ReColorCube, c'est le premier argument
	...
	/***
	 * Cr�ation de la sc�ne - Tous les cubes
	 * @return BranchGroup
	 */
		...
		for(float Z=(-MoinsCenter); Z<numberLineCube-MoinsCenter; Z++)
			for(float Y=(-MoinsCenter); Y<numberLineCube-MoinsCenter; Y++)
				for(float X=(-MoinsCenter); X<numberLineCube-MoinsCenter; X++)
				{
					...
					//Mais, au d�but, on ne peut pas les tourner. Car, on n'utilise pas encore la fonction setSchedulingBounds
...
//		}
		...
		//Creer un panel pour afficher les info correspondant � un cube
		...
		/** Ev�nements JComboBox **/
		
		// S�lection d'un cube dans la liste
		cb_choix_cube.addItemListener(new ItemListener(){
			public void itemStateChanged(ItemEvent e)
			{
				...
				{
					// Retirer de la liste l'item par d�faut
					cb_choix_cube.removeItemAt(0);
					
					// Marche car le nom affich� est un num�ro
...
//					Info info = tab_color_cube[IdNom-1].getInfo();
					...
					{ // Si ce n'est pas la premi�re fois qu'un cube est s�lectionn�
						...
						//Configurer le nouveau cube � faire tourner
